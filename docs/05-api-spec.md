...
평가는 보정된 0~100점 rubric을 사용한다. 점수 계산 전에 답변이 질문에 실질적으로 응답했는지
insufficient gate로 판단한다. 빈 답변은 Request validation에서 차단하며, `"1"`, `"네"`, `"모르겠습니다"`처럼
평가할 내용이 없는 답변과 질문에 무관한 답변은 정확히 0점이다. 짧더라도 질문에 직접 답하는 내용이면
문자열 길이만으로 0점 처리하지 않는다.

AI는 `sufficient`와 category별 criterion score를 구조화 반환하고, Backend가 고정 가중치로 최종 점수를
계산한다. `CS`와 `TECH_STACK`은 요구 충족, 기술 정확성, 설명·근거, 명확성을 사용한다. `EXPERIENCE`는
요구 충족, 상황 구체성, 본인 행동, 기술적 판단, 결과·회고를 사용한다. `SITUATION`과 `COMPANY_FIT`은
각각 상황·행동·판단 및 회사·직무 연결 기준을 적용한다. Criterion score는 현재 Result 계약에 필요하지
않으므로 별도 DB 컬럼에 저장하지 않는다.

점수 앵커는 0=실질적 답변 없음, 1~20=극히 제한적인 관련 내용, 21~40=핵심이 크게 부족한 부분 답변,
41~60=기본적이나 정확성·구체성·근거 부족, 61~80=적절하고 충분한 답변, 81~90=정확하고 구체적인
우수 답변, 91~100=깊이와 근거까지 갖춘 매우 뛰어난 답변이다. Insufficient 답변은 Provider 출력과
관계없이 일관된 0점 feedback으로 보정한다.
